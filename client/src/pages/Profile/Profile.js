...
import { PageContainer, Text } from '../../components/GlobalStyles/PageStyle'
...
const Profile = () => {
    ...
    const updateUser = () => {
        updateProfile({
            ...
            toast.success("User detaials updated.", {
                autoClose: 5500,
                pauseOnHover: true,
            })
        }).catch(err => {
    ...
    return (
        <PageContainer style={{ maxWidth: '800px', marginLeft: 'auto', marginRight: 'auto' }}>
            ...
            <Text style={{ positon: 'relative', width: 'fit-content' }}>
                Hello {user.name}
            ...
            <Fields className='flex'>
                <Fields className='first'>
                    <label>E-Mail</label>
                    <Input value={curUser.User.email}
