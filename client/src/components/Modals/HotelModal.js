...
import { ADD_HOTEL, UPDATE_HOTEL } from '../../graphql/mutations/hotelMutation'
import ImageUpload from '../ImageUpload/ImageUpload';
import { imageUpload } from '../../utils/utilFunctions';
import Loader from '../Loaders/Loader';
import { MAKE_MANAGER } from '../../graphql/mutations/userMutation';
...
const HotelModal = (props) => {
    ...
    const updateCurHotel = async (e) => {
        ...
        if (assignedrooms > hotel.totalRooms) {
            toast.warning(`You have already assigned ${assignedrooms} room. Unassign those rooms to decrease room numbers.`, {
                autoClose: 5500,
                pauseOnHover: true
            })
            return
        ...
        const refPatch = `images/hotels/${hotel.id}/hotelImage`
        let imageUrl = null
                    ...
                    <>
                        ...
                        <form onSubmit={props.action === 'update' ? updateCurHotel : addNewHotel}>
                            <Input required="true" style={{ marginBottom: '16px' }}
                                value={hotel.name} onChange={(e) => setHotel({ ...hotel, description: e.target.value})}
                            ...
                            <Input required="true" style={{ margineBottom: '16px' }}
